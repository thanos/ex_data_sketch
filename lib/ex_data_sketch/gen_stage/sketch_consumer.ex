if Code.ensure_loaded?(GenStage) do
  defmodule ExDataSketch.GenStage.SketchConsumer do
    ...
    available, this module will not be compiled.
    ...
    2. **Raw item**: any other event is passed through `:key_fn` and inserted
       into the accumulated sketch via `sketch_module.update_many/2`. This
       is the contract used by raw event sources (Kafka offsets, Phoenix
       events, etc.).
    ...
    defp ingest_raw_events(acc, raw_events, state) do
      ...
      state.sketch_module.update_many(acc, values)
    end
